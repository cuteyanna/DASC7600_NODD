    ...
    def forward(self, img_features, outputs, targets, size):
        ...
        for i, (query_idx, tgt_idx) in enumerate(indices_list):
            ...
            for n in range(len(original_img_bbox)):
                ...
                if original_img_bbox[n][2] > 1:
                    original_img_bbox[n][2] = 1
                if original_img_bbox[n][3] > 1:
                    original_img_bbox[n][3] = 1

            original_img_bbox = original_img_bbox * \
                           torch.tensor([w, h, w, h], dtype=torch.float32).to(device)

            score = 0
            for m in range(len(original_img_bbox)):
                xmin, ymin, xmax, ymax = original_img_bbox[m,:].long()
                score += torch.mean(up_img_feat[ymin:ymax, xmin:xmax])

            score /= len(original_img_bbox)
            print(score)
...
if __name__ == '__main__':
    ...
    img_feature = torch.randn(N,2048,8,8).sigmoid()
    ...
    for _ in range(N):
        ...
        targets.append(tmp)
    loss_fn = OWDETRLoss(1, 1, 1, 5)
    print(loss_fn(img_features = img_feature,outputs = preds, targets = targets, size=(500, 800)))
