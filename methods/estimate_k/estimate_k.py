...
def test_kmeans_for_scipy(K, merge_test_loader, args=None, verbose=False):
    ...
    for batch_idx, (feats, label, _, mask_lab_) in enumerate(tqdm(merge_test_loader)):
        ...
        #     mask_cls = np.append(mask_cls, np.array([True if x.item() in range(len(args.all_classes))
        #                                              else False for x in label]))
        # else:
        #     mask_cls = np.append(mask_cls, np.array([True if x.item() in range(len(args.train_classes))
        #                                          else False for x in label]))
...
if __name__ == "__main__":
    ...
    # parser.add_argument('--warmup_model_exp_id', type=str, default='(30.10.2022__59.822)') # 用来指示constrastive learning的模型编号，方便查看
    parser.add_argument('--warmup_model_exp_id', type=str, default='(26.11.2022__00.001)_best')
    ...
    args.save_dir = os.path.join(args.root_dir, f'{args.model_name}_{args.dataset_name}') #'./osr_novel_categories/extracted_features_public_impl\\vit_dino_coco'

    args = get_class_splits(args)

    if args.dataset_name == 'coco':
        args.num_labeled_classes = len(args.all_classes) - 1
        args.num_unlabeled_classes = 1  # no used here
    else:
    ...
    #unlabelled_train_examples_test train unlabel的部分用test的transformation
    #test_dataset用来test的dataset，应该和train分开
    ...
    #只用到了train_loader，包含label和unlabel
