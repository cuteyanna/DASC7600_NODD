...
class ConditionalDETR(nn.Module):
    ...
    def forward(self, samples: NestedTensor):
        ...
        ##########################################
        output_class_nc = self.nc_class_embed(hs)

        resnet_1024_feature = src.clone()

        out = {'pred_logits': outputs_class[-1],
               'pred_nc_logits': output_class_nc[-1],
               'pred_boxes': outputs_coord[-1],
               'resnet_1024_feat': resnet_1024_feature}
        if self.aux_loss:
            out['aux_outputs'] = self._set_aux_loss(outputs_class, outputs_coord, output_class_nc)
        ##########################################
        return out

    @torch.jit.unused
    def _set_aux_loss(self, outputs_class, outputs_coord, output_class_nc):
        # this is a workaround to make torchscript happy, as torchscript
        # doesn't support dictionary with non-homogeneous values, such
        # as a dict having both a Tensor and a list.
        return [{'pred_logits': a, 'pred_boxes': b, 'pred_nc_logits': c}
                for a, b, c in zip(outputs_class[:-1], outputs_coord[:-1], output_class_nc[:-1])]
...
class SetCriterion(nn.Module):
    ...
    def loss_NC_labels(self, outputs, targets, indices, num_boxes, current_epoch, owod_targets, owod_indices, log=True):
        ...
        target_classes_o = torch.cat([torch.full_like(t["labels"][J], 0)
                                      for t, (_, J) in zip(owod_targets, owod_indices)])

        # src_logits.shape[:2] -> (16, 300)
        ...
        loss_ce = src_logits.shape[1] * sigmoid_focal_loss(src_logits,
    ...
    def forward(self, samples, outputs, targets, epoch):
        ...
        if epoch >= loss_epoch:
            ...
            for i in range(len(indices)):
                combined = torch.cat((queries, indices[i][0]))
                uniques, counts = combined.unique(return_counts=True)
                unmatched_indices = uniques[counts == 1]  # same as np.setdiff1
        ...
        for loss in self.losses:
            losses.update(self.get_loss(loss, outputs, targets, indices, num_boxes,
                                        epoch, owod_targets, owod_indices))
...
def build(args):
    ...
    if args.masks:
        losses += ["masks"]
    criterion = SetCriterion(args, num_classes=num_classes, matcher=matcher, weight_dict=weight_dict,
