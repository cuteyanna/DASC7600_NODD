...
from data.cocotrue import CocoTrueDataset
...
if __name__ == "__main__":
    ...
    #                     default='./osr_novel_categories/metric_learn_gcd/log/(30.10.2022__59.822)/checkpoints/model.pt')
    parser.add_argument('--warmup_model_dir', type=str,
                        default='./osr_novel_categories/metric_learn_gcd/log/(24.11.2022__00.000)/model.pt')
    parser.add_argument('--use_best_model', type=str2bool, default=True)
    parser.add_argument('--model_name', type=str, default='vit_dino', help='Format is {model_name}_{pretrain}')
    parser.add_argument('--dataset', type=str, default='cocotrue', help='options: cifar10, cifar100, scars')
    ...
    if args.model_name == 'vit_dino':
        ...
        if plat == 'linux':
            model = torch.hub.load(repo_or_dir=r'./facebookresearch_dino_main',
                               model = 'dino_vitb16',
                               source='local',
                               pretrained=False)
        else:
        ...
        # model = torch.hub.load(repo_or_dir=r'C:\Users\fengs/.cache\torch\hub\facebookresearch_dino_main',
        #                        model = 'dino_vitb16',
        #                        source='local',
        #                        pretrained=False)
    ...
    if args.dataset == 'cifar10':
        ...
        train_dataset = CocoDataset(train=True, transform=val_transform)
        # test_dataset = CocoDataset(train=False, transform=val_transform)
        targets = list(set(train_dataset.target))
        targets = [i-1 for i in targets]          # coco are labelled 1 - 21. Change to 0 - 20

    elif args.dataset == 'cocotrue':

        train_dataset = CocoTrueDataset(train=True, transform=val_transform)
        test_dataset = CocoTrueDataset(train=False, transform=val_transform)
        targets = list(set(train_dataset.target))
        targets = [i - 1 for i in targets]          # SCars are labelled 1 - 80. Change to 0 - 79
