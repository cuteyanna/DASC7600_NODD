...
from config import exp_root, dino_pretrain_path, dino_pretrain_path_coco
...
def train(projection_head, model, train_loader, test_loader, unlabelled_train_loader, args): #test_loader, unlabelled_train_loader unused here

    # optimizer = SGD(list(projection_head.parameters()) + list(model.parameters()), lr=args.lr, momentum=args.momentum,
    #                 weight_decay=args.weight_decay)

    optimizer = Adam(list(projection_head.parameters()) + list(model.parameters()),
                     lr=args.lr, betas=(0.9, 0.999), eps=1e-8, weight_decay=args.weight_decay)

    exp_lr_scheduler = lr_scheduler.ExponentialLR(
            optimizer,
            gamma=0.9,
            # start_factor=0.1,
            # total_iters=4
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, batch in enumerate(tqdm(train_loader)):
            ...
            sup_con_labels = class_labels[mask_lab] #0-4

            sup_con_loss = sup_con_crit(sup_con_feats, labels=sup_con_labels) # 对labelled obj做sup_con_labels
        ...
        save_dir = args.model_path[:-3]+"_"+str(epoch)+".pt"
        torch.save(model.state_dict(), save_dir)
        print("model saved to {}.".format(save_dir))

        torch.save(projection_head.state_dict(), save_dir[:-3] + '_proj_head.pt')
        print("projection head saved to {}.".format(save_dir[:-3] + '_proj_head.pt'))
...
if __name__ == "__main__":
    ...
    # parser.add_argument('--warmup_model_dir', type=str, default='./osr_novel_categories/metric_learn_gcd/log/(17.11.2022__14.601)/checkpoints/model_best.pt')
    parser.add_argument('--model_name', type=str, default='vit_dino', help='Format is {model_name}_{pretrain}')
    parser.add_argument('--dataset_name', type=str, default='cocotrue', help='options: cifar10, cifar100, scars')
    ...
    parser.add_argument('--transform', type=str, default='coco')
    ...
    # tensorboard --logdir='.\osr_novel_categories\metric_learn_gcd\log\(17.11.2022__03.394)'
    ...
    """
    coco doesnt need num_labeled_classes/unlabeled_classes since we dont do the test kmeans here
    """
    ...
    if args.base_model == 'vit_dino':
        ...
        args.crop_pct = 0.875
        if args.dataset_name =='coco':
            pretrain_path = dino_pretrain_path_coco
        else:
    ...
    # --------------------
    train_transform, test_transform = get_transform(args.transform, image_size=args.image_size, args=args) #train has 2 more methods compared to test
    ...
    test_loader_unlabelled = DataLoader(unlabelled_train_examples_test, num_workers=args.num_workers,
                                        batch_size=args.batch_size, shuffle=False) # train_unlabelled by test transform
    ...
    #     projection_head, model, train_loader, test_loader, unlabelled_train_loader, args